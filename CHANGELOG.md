...
- **BREAKING — major feature trim** — removed features that added complexity or scope creep for a personal markdown editor: canvas/whiteboard, semantic search, real-time collaboration (Yjs/PartyKit), the plugin system, document library, templates, knowledge graph, comments, reader view, writing goals, the split-pane editor, and DOCX / slides / GitHub-gist export. Also removed the `Open ▾` dropdown, the Quick Open file switcher (`Ctrl+Shift+O`), and the `+` new-tab buttons — new documents are created via File → New (`Ctrl+N`). Dependencies dropped: `yjs`, `y-partykit`, `y-prosemirror`, `@huggingface/transformers`, `docx`, `partykit` (−116 packages).
...
- **Quiet Status Bar** — status bar shows a save-state dot + word count by default. Secondary controls (Writing statistics, Focus mode, Full width, History, About) move into a popover triggered by the ⋯ button. Adds an inline zoom control (− 100% +) that mirrors Ctrl+wheel / Ctrl+± / Ctrl+0.
- **GNOME-style Top Bar** — header restyled to match modern Linux editors with rounded pill controls, a 3-pill theme picker (Light / Dark / Auto), grouped hamburger (☰) menu, and a top-level Info (ⓘ) button. Header height bumped to 46 px.

### Fixed

- **Editor focus ring** — the global `:focus-visible` ring no longer wraps the contenteditable surface, which had rendered as orange left/right bars framing the document.
